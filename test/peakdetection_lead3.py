...
def detect_pqrst(ecg, r_peaks, fs):
    ...
    for r_peak in r_peaks:
        ...
        q_wave = np.argmax(ecg[r_peak - int(0.05 * fs): r_peak]) + (r_peak - int(0.05 * fs))

        # P-wave detection (before R peak)
        p_start = 0 if r_peak - int(0.25 * fs) < 0 else r_peak - int(0.25 * fs)
        p_wave = np.argmin(ecg[p_start: q_wave]) + p_start
        
        # S-wave detection (after R peak)
        s_wave = np.argmax(ecg[r_peak : r_peak + int(0.05 * fs):]) + r_peak
         
        # T-wave detection (after R peak)
        t_wave = np.argmin(ecg[s_wave: r_peak + int(0.25 * fs)]) + s_wave
...
# Example usage
ecg_signal = np.loadtxt('lead3.txt')  # Load your ECG data here
